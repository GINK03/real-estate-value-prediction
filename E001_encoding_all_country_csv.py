...
for val in list(df['kaisou'].unique()):
    if val in [1, 2, 3]:
        df[f'kaisou:{val}'] = df['kaisou'].apply(
            lambda x: 1 if x == val else 0)
    elif val.isdigit() and int(val) >= 3:
        df[f'kaisou:4階以上'] = df['kaisou'].apply(
            lambda x: 1 if str(x).isdigit() and int(x) >= 4 else 0)
    else:
        df[f'kaisou:{val}'] = df['kaisou'].apply(
            lambda x: 1 if x == val else 0)
...
for val in [val//5*5 for val in list(df['chikunen'].unique())]:
    df[f'chikunen:{val}'] = df['chikunen'].apply(
        lambda x: 1 if x//5*5 == val else 0)
df.drop(['chikunen'], axis=1, inplace=True)

# 都道府県情報
for val in list(df['country'].unique()):
    df[f'country:{val}'] = df['country'].apply(
        lambda x: 1 if x == val else 0)
...
for val in list(df['detail_position'].unique()):
    df[f'detail_position:{val}'] = df['detail_position'].apply(
        lambda x: 1 if x == val else 0)
'''
